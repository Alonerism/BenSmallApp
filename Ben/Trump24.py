# time_to_weekly.py
# Parse "Time Activity Report.xlsx" → sum hours per person per day → fuzzy-match to WeeklyTime →
# fill per-day Reg/OT, and build a secretary-friendly Review_Queue + summary message.
...
# =======================
DEFAULTS = dict(
    round_to_hours=0.5,      # round to nearest half-hour (use 0.25 for quarter-hour, or 1.0 for whole hours)
    reg_cap=8.0,             # Regular hours cap per day
    daily_max_hours=16.0,    # sanity cap for a single day
    long_stint_flag=10.0,    # if a single stint >= this, we'll flag "likely missed lunch"
    match_min_score=92,      # fuzzy match threshold (strict)
    fallback_score=85,       # fallback threshold for same-last-name heuristic
    flag_low_weekday=2.0,    # weekday hours <= this (but >0) flagged as "very low weekday"
    suggest_lunch_deduct=0.5 # suggested deduction when we suspect missed lunch (never auto-applied)
)
...
def _build_longest_stint_views(
    *,
    rows_in_block: dict,               # (date, employee) -> [stints...]
    hours_lookup: dict,                # (date, employee) -> rounded hours
    ...
    if not by_day_df.empty:
        # flagged first, then stint desc, then name/date
...
def _build_preview_compact_lists(
    *,
    rows_in_block: dict,                 # (date, employee) -> [stints...]
    hours_lookup: dict,                  # (date, employee) -> rounded total hours for that day/person
    tar_to_wk: Dict[str, Tuple[str,int]],
    round_to_hours: float,
    reg_cap: float,
    long_stint_flag: float,
    suggest_lunch_deduct: float,
    min_stint_for_list: float = 4.0,     # exclude stints < 4h
    ot_threshold: float = 8.0            # OT list only considers totals > 8h
    ...
    for (d, emp), stints in rows_in_block.items():
        if not stints:
            continue
        # longest single stint for that date/person
        longest = max([s for s in stints if isinstance(s, (int, float)) and not math.isnan(s)] or [None])
        total = hours_lookup.get((d, emp), None)

        # resolve display name: Weekly match if present, else TAR name
        wmatch, _ = tar_to_wk.get(emp, (None, 0))
    ...
    # ---------- A) Longest single stint per person (>= min_stint_for_list) ----------
    a = base.dropna(subset=["LongestStint"]).copy()
    a = a[a["LongestStint"] >= float(min_stint_for_list)]
    if not a.empty:
        # choose the row with the highest LongestStint per person (tie-breaker: higher DayTotal, then earliest date)
        a = a.sort_values(["Person","LongestStint","DayTotal","Date"],
                          ascending=[True, False, False, True])
        a = a.drop_duplicates(subset=["Person"], keep="first")
        # suggested lunch reduction when single-stint looks too long
        def _suggest(row):
    ...
    # ---------- B) Max OT day per person (DayTotal > ot_threshold) ----------
    b = base.dropna(subset=["DayTotal"]).copy()
    b = b[b["DayTotal"] > float(ot_threshold)]
    if not b.empty:
        # for each person, keep the day with max total (tie-breaker: larger longest stint, then earliest date)
        b = b.sort_values(["Person","DayTotal","LongestStint","Date"],
                          ascending=[True, False, False, True])
        b = b.drop_duplicates(subset=["Person"], keep="first")
        # compute OT hours as total - reg_cap (>=0)
...
# =======================
def _parse_time_activity(raw_times_path: str):
    """
    Returns:
...
def _read_weekly_structure(weekly_template_path: str, *, open_wb: bool = True):
    """
    Returns:
...
def run_time_to_weekly(
    ...
    *,
    round_to_hours: float       = DEFAULTS["round_to_hours"],
    reg_cap: float              = DEFAULTS["reg_cap"],
    daily_max_hours: float      = DEFAULTS["daily_max_hours"],
    long_stint_flag: float      = DEFAULTS["long_stint_flag"],
    match_min_score: int        = DEFAULTS["match_min_score"],
    fallback_score: int         = DEFAULTS["fallback_score"],
    flag_low_weekday: float     = DEFAULTS["flag_low_weekday"],
    suggest_lunch_deduct: float = DEFAULTS["suggest_lunch_deduct"],
    save: bool = True,
    out_dir: Optional[str] = None
) -> TimeToWeeklyResult:
    # 1) Parse raw times
    daily, rows_in_block = _parse_time_activity(raw_times_path)
    daily = daily.copy()
    daily["RoundedHours"] = daily["RawHours"].apply(lambda x: _round_to(x, round_to_hours))

    # 2) Read Weekly structure (skip workbook if not saving → faster Preview)
    day_map, weekly_rows, start_year, wb, ws, wk_df = _read_weekly_structure(
        weekly_template_path, open_wb=save
    )
    ...
    # 5) Fill WeeklyTime Reg/OT (only if workbook is open)
    filled = 0
    ...
    # 7) Write sheets and file (only when saving)
    output_path = None
    if save and wb is not None:
        def _write_df(sheet_name: str, df: pd.DataFrame):
            if sheet_name in wb.sheetnames:
                wb.remove(wb[sheet_name])  # ensure delete before recreate
                # NOTE: above line uses a temp variable; if you dislike the walrus, use: del wb[sheet_name]
        ...
        if out_dir is None:
            out_dir = os.path.dirname(weekly_template_path) or os.getcwd()
        out_name = f"WeeklyTime_Populated_{datetime.datetime.now().strftime('%m.%d.%y')}.xlsx"
...
def build_secretary_message(
    *,
    ...
    # Flagged count that IGNORES rounding-only (when rounded == 8.0)
    flagged_effective = 0
    ...
    # Low-confidence name matches (unique people)
    low_conf = int((name_matching["Flag"] == "REVIEW").sum()) if not name_matching.empty else 0

    # Reason bullets (exclude noisy ones; force low_name_match to equal low_conf)
    ...
    # Examples: one per person, only fields of concern (friendly labels)
    ...
    def _concerns_for_row(r):
        ...
        # Long Stint (show only if it was flagged)
        segs = []
        ...
        # NOTE: intentionally removed the "Rounded: Xm" section

        # Reduced (our suggestion vs rounded)
